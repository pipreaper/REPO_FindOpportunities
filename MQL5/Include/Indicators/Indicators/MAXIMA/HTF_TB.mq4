...
#property strict
#property description" Find T/B's on HTF according to pegs to count to T/B"
#property description" ... and enclosure of 1,2 or 3 candles on turning"
#property description" reflecting major inter and minor T/B"
#property description" PIN BAR DESCRIPTION IN OHLC_TB"
#property description" If pin bar is a high according to pegs to count T/B"
#property description" ... then "
#property description" bPinBar && (pinBarHigh==high[shift+2]) && (close[shift+1]<low[shift+2]) && (close[shift+1]<low[shift+3])"
...
//| Global variablesADX                                              |
//+------------------------------------------------------------------+
extern ENUM_TIMEFRAMES enumHTFPeriod=PERIOD_M15;//Higher TF
extern int lowHighOffset=30;//pegs to find interest point
extern bool bMajor=true;//Find Majors
extern bool bInter= false;//and/or Minors
extern bool bMinor= false;//and/or Minors
extern bool bPinBar = false;//and/or Pins Bars
...
int OnInit()
  {
   ...
   IndicatorBuffers(3);
   IndicatorShortName("TOPS_BOTTOMS"+" "+instrument+" "+string(enumHTFPeriod));

   SetIndexStyle(0,DRAW_ARROW,0,2,clrLine);
   SetIndexArrow(0,228);
   SetIndexLabel(0,"TB Long_"+string(enumHTFPeriod));
   SetIndexBuffer(0,ExtLongArrow);
   SetIndexEmptyValue(0,EMPTY_VALUE);

   SetIndexStyle(1,DRAW_ARROW,0,2,clrLine);
   SetIndexArrow(1,230);
   SetIndexLabel(1,"TB Short_"+string(enumHTFPeriod));
      ...
      for(shift=limit-1; shift>=0; shift--)//start rates_total down to zero
        {
         ...
         double top=iCustom(instrument,enumHTFPeriod,"OHLC_TB",lowHighOffset,bMajor,bInter,bMinor,bPinBar,0,phtfShift);
         double bottom=iCustom(instrument,enumHTFPeriod,"OHLC_TB",lowHighOffset,bMajor,bInter,bMinor,bPinBar,1,phtfShift);
