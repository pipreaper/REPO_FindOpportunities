      ...
      for(shift=limit-1; shift>=0; shift--)//start rates_total down to zero
        {
         ...
         else if((ExtStatus[shift+1]==1) && (tfCCI>considerationHighLevel))
           {
            ExtStatus[shift+1]=1;
           }
         //CCI turning over at the top     
         else if((tfPrevCCI>levelTop) && (tfCCI<levelTop))
           {
            ExtStatus[shift+1]=-1;
            ExtShortArrow[shift+1]=ExtCCI[shift+1]+MathAbs(tfCCI-tfPrevCCI);
           }
         //CCI Turned and is above and within limits of rec
         else if((ExtStatus[shift+1]==-1) && (tfCCI<considerationLowLevel))
           {
            ExtStatus[shift+1]=-1;
           }
         else
