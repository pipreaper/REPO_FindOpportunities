...
#property description "AS PERCENTILE VOLUME BUT NEED COPY FOR DRAW PERCENT LINES ON HTF_VOL"
#property link      ""
...
string shortName="AUX_PERCENTILE_VOLUME"+" "+Symbol()+" "+string(Period());
...
int OnInit()
  {
   ...
   //if(sName==NULL)
   //  {
   //   Print("is null ",shortName);
   //  }
   //else
if(sName!=NULL)   
 //    {
      shortName=sName;
     // Print("is not null ",shortName);
 //    }

   pLL=-1;pLML=-1;pML=-1;pUML=-1;pUL=-1;

   IndicatorBuffers(2);
   subWindow=ChartWindowFind(ChartID(),shortName);
   uniqueUpperLine="ULine"+shortName;
   uniqueUpperMiddleLine="UMLine"+shortName;
   uniqueMiddleLine="MLine"+shortName;
   uniqueLowerMiddleLine="LMLine"+shortName;
   uniqueLowerLine="LLine"+shortName;
   if(drawLines)
     {
   ...
   for(int shift=0; shift<limit; shift++)
     {
      if(shift>=rates_total-1)
         continue;
      double bufferUp=iCustom(Symbol(),Period(),"volume",3,0,shift);//UP
      double bufferDown=iCustom(Symbol(),Period(),"volume",3,1,shift);//DOWN      
      if(bufferUp!=EMPTY_VALUE)
         ExtValueBuffer[shift]=bufferUp;
      else if(bufferDown!=EMPTY_VALUE)
                          ExtValueBuffer[shift]=bufferDown;
      else
         ExtValueBuffer[shift]=0;
      if(isNewBar && (shift==(limit-2)))
         //make an approximation because dont need to update every tick
         //every new bar is sufficent
        {
         //Print(shift," ",limit);            
...
void OnDeinit(const int reason)
  {
   ObjectDelete(ChartID(),uniqueUpperLine+shortName);
   ObjectDelete(ChartID(),uniqueUpperMiddleLine+shortName);
   ObjectDelete(ChartID(),uniqueMiddleLine+shortName);
   ObjectDelete(ChartID(),uniqueLowerMiddleLine+shortName);
   ObjectDelete(ChartID(),uniqueLowerLine+shortName);
   Sleep(200);
